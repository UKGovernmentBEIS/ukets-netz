...
public class VerifierAuthorityService {
    ...
    public boolean existsNonPendingAuthorityForVB(String userId, Long verificationBodyId) {
    	Optional<Authority> authorityOpt = authorityRepository.findByUserIdAndVerificationBodyId(userId, verificationBodyId);
		return authorityOpt.isPresent() && 
				authorityOpt.get().getStatus() != AuthorityStatus.PENDING;
    }
    
    public boolean existsAuthorityNotForVB(String userId, Long verificationBodyId) {
		return authorityRepository.existsByUserIdAndVerificationBodyIdNotForVBOrNull(userId, verificationBodyId);
	}
    ...
    public String createPendingAuthority(Long vbId, String roleCode, String verifierUser, AppUser authCreationUser) {
        ...
        if (userAuthorityOptional.isPresent()) {
            ...
            if (AuthorityStatus.PENDING.equals(authority.getStatus())) {
                ...
                log.error("Authority for user '{}' in verification body '{}' exists with code '{}' and status'{}'",
