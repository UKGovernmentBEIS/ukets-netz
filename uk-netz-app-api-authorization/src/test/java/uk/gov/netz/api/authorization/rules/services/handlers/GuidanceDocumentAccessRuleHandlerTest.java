...
import uk.gov.netz.api.authorization.rules.services.authorityinfo.providers.GuidanceDocumentAuthorityInfoProvider;
...
class GuidanceDocumentAccessRuleHandlerTest {

    @InjectMocks
    private GuidanceDocumentAccessRuleHandler handler;

    @Mock
    private GuidanceDocumentAuthorityInfoProvider guidanceDocumentAuthorityInfoProvider;
    ...
    void evaluateRules() {
        ...
        when(guidanceDocumentAuthorityInfoProvider.getGuidanceDocumentCaById(sectionId)).thenReturn(competentAuthority);
        handler.evaluateRules(rules, appUser, String.valueOf(sectionId));

        verify(guidanceDocumentAuthorityInfoProvider).getGuidanceDocumentCaById(sectionId);
        verify(appAuthorizationService).authorize(appUser, authorizationCriteria);
        verifyNoMoreInteractions(guidanceDocumentAuthorityInfoProvider, appAuthorizationService);
    }
    ...
    void evaluateRules_resource_forbidden() {
        ...
        when(guidanceDocumentAuthorityInfoProvider.getGuidanceDocumentCaById(sectionId)).thenReturn(resourceCompetentAuthority);
