...
public class TestKafkaCorrelationParentHeaderProducerInterceptor<K, V>
	...
	public ProducerRecord<K, V> onSend(ProducerRecord<K, V> record) {
		if (record.headers().lastHeader(KafkaConstants.CORRELATION_PARENT_ID_HEADER) == null) {
			record.headers().add(KafkaConstants.CORRELATION_PARENT_ID_HEADER, UUID.randomUUID().toString().getBytes());	
		}
		return record;
	}
